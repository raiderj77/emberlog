...
export default function DryBrine() {
  ...
  const numericWeight = boundedNumber(weight, { min: 0.1, max: 1_000 });
  const weightError = numericWeight === null
    ? `Enter a ${unit} weight from 0.1 to 1,000.`
    : null;

  const { grams, tsp } = useMemo(() => {
    if (weightError) return { grams: null, tsp: null };
    const g = unit === "lb" ? numericWeight * 453.592 : numericWeight * 1000;
    const salt = (g * pct) / 100;
    return { grams: salt, tsp: salt / SALT[brand] };
  }, [numericWeight, unit, pct, brand, weightError]);
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block"><span className="mb-1 block text-sm font-medium">Meat weight</span>
          <div className="flex gap-2">
            <input type="number" min="0.1" max="1000" step="0.1" value={weight} onChange={(e) => setWeight(e.target.value)} aria-invalid={weightError ? "true" : undefined} aria-describedby={weightError ? "dry-brine-weight-error" : undefined} className="inp" />
      ...
      {weightError && <p id="dry-brine-weight-error" role="alert" className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm font-medium text-rose-800">{weightError}</p>}

      {!weightError && <div className="grid gap-4 sm:grid-cols-2" aria-live="polite" aria-atomic="true">
      ...
      <div className="rounded-lg border border-line bg-paper p-4 text-sm text-muted">
        ...
        <p className="mt-1">Keep the refrigerator at 40°F or colder. Use a rack over a rimmed pan on a low shelf below ready-to-eat food, contain raw-meat drips, and sanitize contact surfaces. Typical windows: 1-12 hr for chops and chicken pieces, 12-24 hr for a whole chicken or roast, and 24-48 hr for turkey. More detail in the <Link href="/guides/dry-brine-guide/" className="font-semibold text-ember-600 underline">dry brine guide</Link>.</p>
      </div>
